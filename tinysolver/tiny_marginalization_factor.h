#ifndef SLAMTOOL_TINY_MARGINALIZATION_FACTOR_H
#define SLAMTOOL_TINY_MARGINALIZATION_FACTOR_H
...
#include <slamtool/estimation/marginalization_factor.h>

namespace slamtool {

class TinyMarginalizationFactor : public MarginalizationFactor, public ceres::CostFunction{
  public:
    TinyMarginalizationFactor(Map *map) : MarginalizationFactor(map) {
    ...
    void marginalize(size_t index) override {
        ...
        const size_t FRAME_NUM = base_map->frame_num() - 2;

        pose_motion_infomat.resize(FRAME_NUM * ES_SIZE, FRAME_NUM * ES_SIZE);
        pose_motion_infovec.resize(FRAME_NUM * ES_SIZE);
        ...
        for (size_t i = 0; i < FRAME_NUM; ++i) {
            if (i < index) {
                ...
                frame_indices[base_map->get_frame(i)] = FRAME_NUM - 1;
            }
        }
        ...
        /* scope: marginalization factor */
        {
            ...
            Evaluate(marparameters.data(), marresidual.data(), marjacobian_ptrs.data());
            ...
            for (size_t i = 0; i < frames.size(); ++i) {
                ...
                for (size_t j = 0; j < frames.size(); ++j) {
                    ...
                    pose_motion_infomat.block<ES_SIZE, ES_SIZE>(ES_SIZE * frame_index_i, ES_SIZE * frame_index_j) += state_jacobians[i].transpose() * state_jacobians[j];
                }
                pose_motion_infovec.segment<ES_SIZE>(ES_SIZE * frame_index_i) += state_jacobians[i].transpose() * marresidual;
            }
        }
        ...
        for (size_t j = index; j <= index + 1; ++j) {
            ...
            std::unique_ptr<TinyPreIntegrationErrorFactor> picost = std::make_unique<TinyPreIntegrationErrorFactor>(frame_i, frame_j, frame_j->keyframe_preintegration);
            ...
            dr_dstates_i.block<ES_SIZE, 3>(0, ES_Q) = dr_dqi.block<ES_SIZE, 3>(0, 0);
            ...
            dr_dstates_j.block<ES_SIZE, 3>(0, ES_Q) = dr_dqj.block<ES_SIZE, 3>(0, 0);
            ...
            pose_motion_infomat.block<ES_SIZE, ES_SIZE>(ES_SIZE * frame_i_index, ES_SIZE * frame_i_index) += dr_dstates_i.transpose() * dr_dstates_i;
            pose_motion_infomat.block<ES_SIZE, ES_SIZE>(ES_SIZE * frame_i_index, ES_SIZE * frame_j_index) += dr_dstates_i.transpose() * dr_dstates_j;
            pose_motion_infomat.block<ES_SIZE, ES_SIZE>(ES_SIZE * frame_j_index, ES_SIZE * frame_i_index) += dr_dstates_j.transpose() * dr_dstates_i;
            pose_motion_infomat.block<ES_SIZE, ES_SIZE>(ES_SIZE * frame_j_index, ES_SIZE * frame_j_index) += dr_dstates_j.transpose() * dr_dstates_j;
            pose_motion_infovec.segment<ES_SIZE>(ES_SIZE * frame_i_index) += dr_dstates_i.transpose() * piresidual;
            pose_motion_infovec.segment<ES_SIZE>(ES_SIZE * frame_j_index) += dr_dstates_j.transpose() * piresidual;
        }
        ...
        for (size_t j = 0; j < frame_victim->keypoint_num(); ++j) {
            ...
            for (const auto &[frame_tgt, keypoint_index] :
                ...
                std::unique_ptr<TinyReprojectionErrorFactor> rpcost = std::make_unique<TinyReprojectionErrorFactor>(frame_tgt, track);
        ...
        /* scope: marginalize the corresponding frame */ {
            size_t last_index = FRAME_NUM - 1;
            matrix<15, 15> inv_infomat = pose_motion_infomat.block<ES_SIZE, ES_SIZE>(ES_SIZE * last_index, ES_SIZE * last_index).inverse();
            matrix<> complement_infomat;
            vector<> complement_infovec;
            complement_infomat.resize(ES_SIZE * (FRAME_NUM - 1), ES_SIZE * (FRAME_NUM - 1));
            complement_infovec.resize(ES_SIZE * (FRAME_NUM - 1));
            complement_infomat.setZero();
            complement_infovec.setZero();

            matrix<> complement_infomat_block = pose_motion_infomat.block(0, 0, ES_SIZE * last_index, ES_SIZE * last_index);
            vector<> complement_infovec_segment = pose_motion_infovec.segment(0, ES_SIZE * last_index);
            complement_infomat_block -= pose_motion_infomat.block(0, ES_SIZE * last_index, ES_SIZE * last_index, ES_SIZE) * inv_infomat *
                pose_motion_infomat.block(ES_SIZE * last_index, 0, ES_SIZE, ES_SIZE * last_index);
            complement_infovec_segment -= pose_motion_infomat.block(0, ES_SIZE * last_index, ES_SIZE * last_index, ES_SIZE) * inv_infomat *
                pose_motion_infovec.segment(ES_SIZE * last_index, ES_SIZE);
            complement_infomat.block(0, 0, ES_SIZE * last_index, ES_SIZE * last_index) = complement_infomat_block;
            complement_infovec.segment(0, ES_SIZE * last_index) = complement_infovec_segment;
        ...
        /* scope: create marginalization factor */ {
            ...
            infovec = lambdas_inv.cwiseSqrt().asDiagonal() * saesolver.eigenvectors().transpose() * pose_motion_infovec;
            frames.resize(FRAME_NUM - 1);
            pose_linearization_point.resize(FRAME_NUM - 1);
            motion_linearization_point.resize(FRAME_NUM - 1);
            set_num_residuals((int)frames.size() * ES_SIZE);
            mutable_parameter_block_sizes()->clear();
            for (size_t i = 0; i < FRAME_NUM; ++i) {
                if (i == index)
...
#endif // SLAMTOOL_TINY_MARGINALIZATION_FACTOR_H
