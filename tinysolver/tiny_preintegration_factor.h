#ifndef SLAMTOOL_TINY_PREINTEGRATION_FACTOR_H
#define SLAMTOOL_TINY_PREINTEGRATION_FACTOR_H

#include <slamtool/common.h>
#include <slamtool/estimation/preintegration_factor.h>
#include <slamtool/geometry/lie_algebra.h>
#include <slamtool/map/frame.h>

namespace slamtool {

class TinyPreIntegrationErrorFactor: public PreIntegrationErrorFactor {

public:
    TinyPreIntegrationErrorFactor(Frame *frame_i, Frame *frame_j, const PreIntegrator &preintegration)
        : frame_i(frame_i), frame_j(frame_j), preintegration(preintegration) {}

    inline bool Evaluate(const double *const *parameters, double *residuals, double **jacobians) const {
        static const vector<3> gravity = {0.0, 0.0, -SLAMTOOL_GRAVITY_NOMINAL};
...
class TinyPreIntegrationPriorFactor :public PreIntegrationPriorFactor{
public:
    TinyPreIntegrationPriorFactor(Frame *frame_i, Frame *frame_j, const PreIntegrator &preintegration)
    :piefactor(frame_i, frame_j, preintegration) {}

    TinyPreIntegrationErrorFactor piefactor;
};

} // namespace slamtool

#endif // SLAMTOOL_CERES_PREINTEGRATION_FACTOR_H
